...
import { Briefcase, Hash, Lock, Eye, EyeOff, ArrowLeft } from 'lucide-react'
import { getEmployeeByCredentials, logActivity } from '../api/rbac'
import { setEmployee } from '../hooks/useAuth'
...
export default function EmployeeLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const { data, error: dbError } = await getEmployeeByCredentials(form.employee_id.trim(), form.password)
      if (dbError) throw dbError
      if (data) {
        setEmployee(data)
        await logActivity({ actor_type: 'employee', actor_id: data.id, actor_name: `${data.first_name} ${data.last_name}`, action: 'Adviser login', entity_type: 'session', entity_id: data.id, details: { employee_id: data.employee_id } }).catch(() => {})
        navigate('/adviser/portal', { replace: true })
      } else {
    ...
    <div className="lp-root">
      ...
      <div className="lp-layout">
        <div className="lp-left">
          ...
          <div className="lp-tagline">Adviser Portal</div>
          <div className="sl-info-cards">
            <div className="sl-info-card"><Briefcase size={18}/><span>View assigned students</span></div>
            <div className="sl-info-card"><Briefcase size={18}/><span>Post announcements & activities</span></div>
            <div className="sl-info-card"><Briefcase size={18}/><span>Input and update grades</span></div>
          </div>
        </div>
        ...
        <div className="lp-right">
          <div className="lp-card" ref={cardRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
            ...
            <h2 className="lp-card-title">Adviser Login</h2>
            <p className="lp-card-sub">Sign in with your Employee ID and password</p>
            ...
            <form onSubmit={handleSubmit} className="lp-form">
              <div className="lp-field">
                ...
                <div className="lp-input">
                  ...
                  <input name="employee_id" value={form.employee_id} onChange={handle} placeholder="e.g. EMP-001" required />
                </div>
              </div>
              <div className="lp-field">
              ...
              <button type="submit" className="lp-btn" style={{ background: 'linear-gradient(135deg,#e8650a,#c45200)' }} disabled={loading}>
                {loading ? <><span className="spinner" /> Signing in...</> : 'Access Adviser Portal'}
              </button>
            </form>
